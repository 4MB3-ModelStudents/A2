...
By fitting a line through the component of the log transformed curve that is increasing towards the peak, the parameter B can be estimated by calculating the slope of the linear relationship, and the parameter I0 by calculating the y-intercept of the line. The parameter B represents the average number of contacts with the susceptible population that lead to a new infective per unit time per infective in the population. Using the estimated value of B, the parameter \beta can be estimated by dividing B by the number of susceptible individuals within the population. To estimate the number of susceptible individuals within the population, one can assume that he value of the entire population of Philadelphia before the epidemic began. This assumption does not take into consideration the inherit variability within population based on immigration, births, and deaths. 
When the removed state is not considered within the model, the prevalence showcases a bounded exponential relationship. This means that once the number of infected individuals reaches a specific threshold, the prevalence of the disease will increase exponential until the number of infected individuals is approximately half of the population. 
The last two graphs compared a variety of curves generated by fixed values of \gamma, S0, and I0, but differed by their \beta values. As a result, each curve was characterized by a different R0 value. When \gamma was set to 1/4, there does not appear to be a value of R0 that produces a curve that fits the data. However, when /gamma is set to _ and \I0 is set to _,  the curve with an R0 value of 5 intercepts the component of the data that is increasing towards the peak almost entirely. R0 represents the basic reproduction number, meaning that it is the average number of secondary cases that are caused by a primary case. To calculate this value, it is assumed that the initial population is entirely susceptible to the infection. 
Since the data collected only account for removable from the population caused by the death, a subset of the data required to define the variable remove is missing from the construction of the corresponding SIR model. To account for the incompleteness of the data, it is highly recommended that PHAC continues to receive funding in order to gather more information from patient records.
