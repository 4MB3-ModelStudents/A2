...
#Plotting data with a grey line, L shape plot borders, and no labels
plot(pim~date,philadata,type="l",col="gray",bty="L",xlab="",ylab="",xaxs="i",yaxs="i",las=1)
#Plotting red dots with black outlines at data points
...
\item Fit a straight line to the part of the Philadelphia 1918 mortality time series that looks straight on a logarithmic scale (and show your result in a plot).  Once you get the hang of it, the easiest way to do this is to use the \code{lm()} function in \Rlogo (lm stands for linear model).  Note that simplest way to draw a straight line with given slope and intercept is with the \code{abline()} function.  If you find \code{lm()} counter-intuitive to understand then experiment with \code{abline()} until your eyes tell you that you have discovered a line that provides a good fit.
  ...
  {\color{blue} \begin{proof}[Solution]
  {\color{magenta}
<<Q2b,echo=TRUE,fig.height=6,fig.align="center">>=
  #Plotting data with a grey line, L shape plot borders, 
  #and no labels
plot(log(pim)~date,philadata,type="l",col="gray",bty="L",xlab="",
...
mtext("Date",side=1,line=2,adj=1,col="blue",cex=1.25)
loc=par("usr")
text(loc[1],loc[4],"P&I Deaths",xpd=T,adj=c(1,0),pos=3,col="blue",
     cex=1.25)

#Fitting a straight line using lm to a subset of the data
...
#Calculating the mean distance between data 
  ...
  #making vecotr of zeros to store distances betwwen points in
...
By setting $\gamma = 0.25$ this method leads to the following results, 
	...
	$\\R_0 = 1.9756$
	$\\I_0 = $
This fits the given dataset, since the magnitude of $\R_0$ is greater than 1, which implies that $\abs{\frac{dI}{dt}} > 0$. Comparing a variety of curves of from the SIR model with the same $\gamma$, and $\\I_0 $ value and with different values of $\R_0$ ranging from 1 to 5, it can be seen that the curve that fits the data best is the curve with $\R_0 = 2$. Based on these results, it is important that the PHAC continues to receive funding in order to determine better means to contain the growth of influenza in case of an outbreak.
  }
  \end{proof}
  }
