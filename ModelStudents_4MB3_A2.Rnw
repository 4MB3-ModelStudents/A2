...
\item The observed mortality time series $M(t)$ is certainly not equal to the prevalence $I(t)$ that appears in the SIR model.  Suppose, however, that $I(t) = \eta M(t-\tau)$ for all time (where $\eta$ and $\tau$ are constants), \emph{i.e.,} that the mortality curve is exactly a scaled and translated version of the prevalence curve.  Prove that if both $I$ and $M$ are growing exactly exponentially over some time period then their exponential rates are identical.  Thus, if we compare them during the ``exponential phase'' on a logarithmic scale, then both curves will be perfectly straight with exactly the same slope.
 ...
 {\color{blue} \begin{proof}[Solution]
 {\color{magenta}
 Assume that $I(t)$ and $M(t)$ are growing exponentially for $t \in (\alpha, \beta)$ for $\alpha$,$\beta$ $\in$ $\mathbb{R}$:
\begin{align}
  I(t) &= a_1 e^{b_1 t}  \\
  M(t) &= a_2 e^{b_2 t} 
\end{align}
...
<<Q2c,echo=FALSE,results="hide">>=
## get slope
summary(lmresult <- lm(log(pim)~date,loglinedata))
slope <- coef(lmresult)["date"]

est <- 4*as.numeric(slope) + 1 ## calculate estimate of R_0
@

Given $\frac{1}{\gamma} = 4$, and taking the slope to be approximately \Sexpr{paste(signif(slope,4))}, the value of $\R_0$ can be estimated.
\begin{align}
\Sexpr{paste(signif(slope,4))} &\approx (\R_0-1)\gamma \\
\R_0 &\approx 4(\Sexpr{paste(signif(slope,4))}) + 1 \\
\R_0 &\approx \Sexpr{paste(signif(est,4))}
\end{align}
    }
  \end{proof}
  }
...
\item For $I_0=10^{-3}$ and $S_0=1-I_0$, plot the solutions of the SIR model assuming $1/\gamma=4$ days and $\R_0\in\{1.2,1.5,1.8,2,3,4\}$.  Use the \code{legend()} command to make a legend on the plot that shows which curves correspond to which values of $\R_0$.
  ...
  <<Q3c,fig.height=6,fig.align="center">>=
## Vector Field for SIR Model
SIR.vector.field <- function(t,vars,parms=c(R_0=1,gamma=1)) {
  with(as.list(c(parms,vars)), {
    dx <- -R_0*gamma*x*y  ## dS/dt
...
\item By trial and error, find values of $\R_0$ and $\gamma$ that yield a solution of the SIR model that fits the Philadelphia P\&I times series reasonably well.  You can assess the quality of fit using the Euclidean distance between the model solution and the data.  (\emph{Note:}  The trial and error approach is a valuable exercise, but not a suggestion of a method you would really use in practice.  We'll discuss better methods for fitting ODE models to data later.)
  ...
  {\color{blue} \begin{proof}[Solution]
  {\color{magenta} To transform the data so that it is comparable with the scale of the generated models, the x-axis ("Date") must be transformed in terms of time and the y-axis ("P$\&$I Deaths") must be transformed to a prevalence measurement. By taking an index of days after \verb|"1918-09-01"|, the data can be viewed in terms of days which can be compared similarly to the generated solution, which is in terms of mean infectious periods ($\frac{1}{\gamma}$). 
  Since it was proven in question 2(a) that prevalence and mortality rates grow identically, dividing each mortality measurement by a total population will give prevalence in terms of mortality. From Assignment 1, peak prevalence was defined as:

\begin{equation}
I_p = (I_0+S_0) - \frac{1}{\R_0} [\ln{\R_0 S_0}+1]
\end{equation}
Since $I_p = \frac{Peak Deaths}{Total Population}$, an expression for total population in terms of $\R_0$, $I_0$, and $S_0$ (with $S_0 = 1-I_0$) can be found.

\begin{align}
\frac{Peak Deaths}{Total Population} &= (I_0+S_0) - \frac{1}{\R_0} [\ln{\R_0 S_0}+1] \\
Total Population &= \frac{Peak Deaths}{(I_0+S_0) - \frac{1}{\R_0} [\ln{\R_0 S_0}+1]} \\
&= \frac{Peak Deaths}{1 - \frac{1}{\R_0} [\ln{\R_0 (1-I_0)}+1]}
\end{align}

Taking the values used for the estimate, $\R_0 \approx 2$ and $I_0 = 1e^{-6}$:
\begin{align}
Total Population &\approx \frac{803}{1 - \frac{1}{2} [\ln{2(1-1e^{-6})}+1]} \\
&\approx \Sexpr{round(803/(1-(1/2)*(log(2*(1-1e-06))+1)))}
\end{align}
So, according to the model, N=5000 should give a reasonable approximation for the total population. Yet, this only accounts for approximately $\frac{1}{400}$ of the total population of Philadelphia at the time ($\sim$ 2 000 000 people)\cite{pbs}. Further investigations will be required to account for this discrepancy.
<<Q3d,fig.height=6,fig.align="center">>=
## get data in terms of days after 1918-09-01
days <- c(1:length(philadata$date))

## generate box with plot of prevalence in terms of mortality vs days
## zoom into peak interval
plot((pim/5000)~days,philadata,xlim=c(25,55),
     ylim=c(0,0.25),type="l",col="gray",
     bty="L",xlab="",ylab="",xaxs="i",yaxs="i",
     las=1)
## generate datapoints
points((pim/5000)~days,philadata,pch=21,
       bg="red",xpd=TRUE)
## label x-axis
mtext("Days",side=1,line=2,adj=1,col="blue",
      cex=1.25)
## label y-axis
loc=par("usr")
text(loc[1],loc[4],"Prevalence",xpd=T,
     adj=c(1,0),pos=3,col="blue",cex=1.25)
...
S0 <- 1-I0
## draw solutions for several values of parameter R_0
vals <- seq(1.98,2.05,length=5)
line <- c(1:length(vals))
for (i in 1:length(vals)) {
  draw.soln(ic=c(x=S0,y=I0),tmax=tmax,
            func=SIR.vector.field,
            parms=c(R_0=vals[i],gamma=1/3),
            lty=line[i], # use a different line style for each solution
            col=line[i]) # use a different line colour for each solution
}
# draw legend for different values of R_0
legend("topright",legend=vals,lty=line,
       col=line,title="\\R_0 Values")

## Define Euclidean Distance Function
euc.dist <- function(x1=c(1:length(days)), x2=c(1:length(days))) { 
  sqrt(sum((x1[i] - x2[i])^2)) 
}

## Define Distance between Data and Solution
error <- function(ic=c(x=S0,y=I0),tmax=tmax,
                  func=SIR.vector.field,
                  parms=c(R_0=1,gamma=1)) {
## get prevalence in terms of mortality from dataset
  datayvals <- philadata$pim/5000
  ## generate approximated solution for \R_0 and \gamma 
  soln <- ode(ic,times=seq(0,tmax,by=tmax/500),func,parms)
  ## get sample values of time and prevalence in terms of days
  solnxvals <- soln[seq(1,500,4),"time"]
  solnyvals <- soln[seq(1,500,4),"y"]
  ## get euclidean distance by difference of datavalues
  distx <- euc.dist(days,solnxvals)
  disty <- euc.dist(datayvals,solnyvals)
  
  err <- c(distx,disty)
  return(err)
}

## generate error for \R_0 and \gamma value
ans <- error(ic=c(x=S0,y=I0),tmax=tmax,
             func=SIR.vector.field,
             parms=c(R_0=1.9975,gamma=1/3))
@
So, the error in time between the data and the model with $\R_0 \approx 1.9975$ and $I_0 = 1e^{-6}$ is \Sexpr{paste(ans[1])} days and the error in prevalence is \Sexpr{paste(signif(ans[2],4))}.
  }
  \end{proof}
  }
...
\bibitem{pbs} American Experience. \emph{City Snapshots: Philadelphia} 1998. 1918 Influenza. http://www.pbs.org/wgbh/americanexperience/features/general-article/influenza-philadelphia/ \textit{American Experience}
\end{thebibliography}
